...
parser.add_argument("-b","--batch_num_per_class",type = int, default = 1)#Tongji有600个类(左右手看做不同类)，每个类总共20个样本，一般取15
parser.add_argument("-e","--episode",type = int, default= 2000)#500000
parser.add_argument("-v","--valid_episode", type = int, default = 100)
parser.add_argument("-t","--test_episode", type = int, default = 600)
parser.add_argument("-l","--learning_rate", type = float, default = 0.001)
parser.add_argument("-g","--gpu",type=int, default=0)
parser.add_argument("-u","--hidden_unit",type=int,default=10)
parser.add_argument("-sp","--save_path",type = str, default='XJTU_Conv4/models/RelaNet')
...
VALID_EPISODE = args.valid_episode
...
def main():
    ...
    # if os.path.exists(str("Tongji_DeNet/models/DeNet_feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
    #     feature_encoder.load_state_dict(torch.load(str("Tongji_DeNet/models/DeNet_feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
    #     print("load feature encoder success")
    # else:
    #     print("none existing feature encoder")
    # if os.path.exists(str("Tongji_DeNet/models/Meta_ProbeProtypical_network_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
    #     relation_network.load_state_dict(torch.load(str("Tongji_DeNet/models/Meta_ProbeProtypical_network_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")))
    #     print("load relation network success")
    # else:
    #     print("none existing relation network")
    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%200 == 0:#5000
            ...
            for i in range(VALID_EPISODE):
                total_rewards = 0
                task = tg.MiniImagenetTask(metavalid_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,BATCH_NUM_PER_CLASS)
                sample_dataloader = tg.get_mini_imagenet_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False)
                # num_per_class = 5
            ...
            if valid_accuracy > last_accuracy:
                ...
                torch.save(relation_network.state_dict(),str(SAVE_PATH + "/relation_network_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS)+"shot_"+"max-valid.pkl"))
                last_accuracy = valid_accuracy
                print("saved networks and for episode:",episode+1)

    # 开始测试
    # Step 1: init data folders
    print("Begining " + str(CLASS_NUM) +" Way " + str(SAMPLE_NUM_PER_CLASS) + " Shot Max-Valid Models Test")
    # init character folders for dataset construction
    ...
    if os.path.exists(str(SAVE_PATH + "/feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) + "shot_" + "max-valid.pkl")):
        feature_encoder.load_state_dict(torch.load(str(SAVE_PATH + "/feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS)+"shot_"+"max-valid.pkl")))
        relation_network.load_state_dict(torch.load(str(SAVE_PATH + "/relation_network_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS)+"shot_"+"max-valid.pkl")))
