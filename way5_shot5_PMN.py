...
parser.add_argument("-p","--save_path",type = str, default='XJTU_Conv4/models/PMN')
# CNNEncoder主网络设置
parser.add_argument("-e","--episode",type = int, default =8000)#8000
...
parser.add_argument('--unrolling_steps', default=4, type=int)
parser.add_argument('--distance', default='l2')
parser.add_argument('--epsilon', type = float, default = 1e-8)
parser.add_argument('--image_size', type = float, default = 84)
parser.add_argument('--hallucinator_dim', type = int, default = 512)
# 参数及实验结果保存在字典
...
class MLP(nn.Module):
    ...
    def forward(self, input):
        ...
        output = self.relu(out3)
        return output

class MatchingNetwork(nn.Module):
    def __init__(self, n: int, k: int, q: int, fce: bool, num_input_channels: int,
                 lstm_layers: int, lstm_input_size: int, unrolling_steps: int, feature_dim: int, h_dim:int, img_size:int,device: torch.device):
        ...
        self.q = q
        self.fce = fce
        self.in_chan = num_input_channels
        self.img_size = img_size
        self.h_dim = h_dim
        self.img_flaten_dim = self.img_size*self.img_size*self.in_chan
        self.encoder = get_few_shot_encoder(self.in_chan, feature_dim).to(device)
        self.hallucinator = MLP(self.img_flaten_dim*2, self.h_dim, self.img_flaten_dim).to(device)
        if self.fce:
...
def main():
    ...
    model = MatchingNetwork(SAMPLE_NUM_PER_CLASS, CLASS_NUM, BATCH_NUM_PER_CLASS, args.fce, args.num_input_channels,
        lstm_layers=args.lstm_layers,lstm_input_size=args.lstm_input_size,unrolling_steps=args.unrolling_steps,
        feature_dim=args.feature_dim,h_dim=args.hallucinator_dim,img_size=args.image_size,device=GPU)
    ...
    for episode in range(EPISODE):
        ...
        # 特征分解decomposition
        samples_flat = samples.view(samples.shape[0],-1).cuda(GPU)
        noise = Variable(torch.zeros(samples_flat.shape)).data.normal_(0, std=0.1).cuda(GPU)
        #torch.randn(30, 3, 84, 84)
        
        #利用噪声和原图像生成新样本
        hallucinator_input = torch.cat((samples_flat,noise),dim=1)
        hallucinator_output = model.hallucinator(hallucinator_input)
        augted_samples_flat = torch.stack((samples_flat,hallucinator_output),1).view(2*samples_flat.shape[0],-1)
        augted_samples = augted_samples_flat.view(-1,args.num_input_channels,args.image_size,args.image_size)

        support = model.encoder(augted_samples.cuda(GPU))
        ...
        if args.fce:
            ...
            support = support.squeeze(1).view(CLASS_NUM,SAMPLE_NUM_PER_CLASS*2,-1)
            support = torch.sum(support,1).squeeze(1)/(SAMPLE_NUM_PER_CLASS*2)
        ...
        y_pred = tg.pairwise_distances(queries, support, args.distance, args.epsilon)
        ...
        train_loss.backward()
        clip_grad_norm_(model.parameters(), 1.3)
        # Take gradient step
        ...
        if (episode+1)%50 == 0:
            ...
            print(f'Episode {episode+1} in EPISODE {EPISODE}: trian_loss {train_loss.item()}')
        ...
        if (episode+1)%500 == 0:#5000
            with torch.no_grad():
                ...
                for i in range(VALID_EPISODE):
                    ...
                    #利用噪声和原图像生成新样本
                    samples_flat = base.view(samples.shape[0],-1).cuda(GPU)
                    noise = Variable(torch.zeros(samples_flat.shape)).data.normal_(0, std=0.1).cuda(GPU)#torch.randn(30, 3, 84, 84)   
                    ...
                    valid_acc_temp = torch.eq(y_pred.argmin(dim=-1), query_labels.cuda(GPU)).sum()/y_pred.shape[0]
    ...
    for i in range(TEST_EPISODE):
        ...
        test_acc_temp = torch.eq(y_pred.argmin(dim=-1), query_labels.cuda(GPU)).sum()/y_pred.shape[0]
