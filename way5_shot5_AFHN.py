...
path = database + '_Conv4/models/AFHN'
batch_size = 128
...
class MiniImageNet(Dataset):
    def __init__(self, setname):
        ...
        divide = int(3*sample_num_class/4)
        if setname == "train":
          classes = osp.join(ROOT_PATH, "train")
          for cls in os.listdir(classes):
              lb += 1
              for cls_individual in os.listdir(os.path.join(classes, cls))[:divide]:
                  img_path = os.path.join(classes, cls, cls_individual)
                  data.append(img_path)
                  label.append(lb)
          classes = osp.join(ROOT_PATH, "valid")
          for cls in os.listdir(classes):
              lb += 1
              for cls_individual in os.listdir(os.path.join(classes, cls))[:divide]:
                  img_path = os.path.join(classes, cls, cls_individual)
                  data.append(img_path)
                  label.append(lb)
          classes = osp.join(ROOT_PATH, "test")
          for cls in os.listdir(classes):
              lb += 1
              for cls_individual in os.listdir(os.path.join(classes, cls))[:divide]:
                  img_path = os.path.join(classes, cls, cls_individual)
...
class ConvNet(nn.Module):
    def __init__(self, x_dim=3, hid_dim=64, z_dim=64):#输入输出通道数
        ...
        self.fc = nn.Linear(self.out_channels, train_class_num)
    def forward(self, x):
        x = self.encoder(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x#每一个样本的特征为行向量
...
# resnet18 = ConvNet().cuda()

# #We will train on only the 80 classes
# resnet18.fc = nn.Linear(resnet18.fc.in_features, train_class_num)

# #optimizer for training 
# feat_optimizer = optim.Adam(resnet18.parameters(), lr=0.0001, betas=(0.5, 0.999))
...
#   os.makedirs(path)
# torch.save({'F': resnet18.state_dict(),}, str(path+"/FeatureExtractor.pkl"))
...
#load model
checkpoint = torch.load(str(path+"/FeatureExtractor.pkl"))
...
def getDataLoaders(feature_dataset_map,k,n,start_class) :
  ...
  dataloaderS = torch.utils.data.DataLoader(s,  batch_size=len(s), shuffle=False, num_workers=1)
  dataloaderQ = torch.utils.data.DataLoader(q,  batch_size=len(q), shuffle=False, num_workers=1)
...
# s,q = getDataLoaders2(train_features_map, test_features_map,10,5,train_class_num)


class Generator(nn.Module):
    def __init__(self, feat_size):
        super(Generator, self).__init__()
        self.label_embeddings = nn.Embedding(train_class_num, feat_size)
...
optimizer_G = torch.optim.Adam(generator.parameters(), lr=0.000001)
...
lambda_gp = 9 #9
n_epochs = 200 #200
n_classes = train_class_num

batch_size = N*K

end_class = train_class_num
for epoch in range(n_epochs):
    batch =0
    num_random_samples = 1#1
    #################
        ...
        for (feats, labels), (Q_feats, Q_labels) in zip(dataloaderS, dataloaderQ):
            ...
            if batch % 4 == 0:#5for5shot
                ...
                print(
                    ...
                    % (epoch, n_epochs, batch, 4000 / (N * K) , d_loss.item(), g_loss.item())
                )
                torch.save({
...
checkpoint = torch.load(str(path+"/GAN.pkl"))
...
def testFSL(N, K, extra_k):
  end_class = total_class_num
  n_epochs = 20
  n_classes=total_class_num
  ...
  task = 0
  for _ in range(10):
    start_class = train_class_num + 40
    while(start_class < end_class):
      ...
      for (feats, labels), (Q_feats, Q_labels) in zip(dataloaderS, dataloaderQ):
        ...
        accuracy.append(acc_Q) 
        print("FSL Task : ", task, "Total Loss on Training", loss_S, "Accuracy on Query set", acc_Q)
      start_class += N
      del netF
  Accuracy, h = tg.mean_confidence_interval(accuracy)
  print("Total Accuracy on ", task, "Accuracy", Accuracy,", h", h)
  return {"train_Loss" : loss_S, "accuracy": Accuracy, "h": h, "tasks": task}



N = 5
Ks = [1]#[5, 1]
extra_ks  = [1]#[20, 60, 100, 140]for5shot,[1, 5]for1shot
values = {}
for K in Ks:
  values[K] = {}
...
for K in Ks:
  for extra_k in extra_ks:
    print("Value[%d][%d]: train_Loss %f accuracy %f h %f task %f"
              % (K, extra_k, values[K][extra_k]["train_Loss"], values[K][extra_k]["accuracy"], values[K][extra_k]["h"], values[K][extra_k]["tasks"]))
...
def display_result(data) :
  ...
  for K in data :
    ...
    for extra_k in data[K] :
      ...
      X.append(extra_k)
      Y.append(accuracy)
    axarr[i].set_ylim(ymin=0.4,ymax = 1.0)
    axarr[i].set_ylim(ymin=0.4,ymax = 1.0)
  ...
  # plt.show()
  # plt.savefig(path + '/' + str(lambda_gp) + str(num_random_samples) + '.png')
  # plt.close()
  plt.show()
